...
class df_ColumnEditor:
    ...
    def confirm_changes(self):
        ...
        if self.top_listbox_edits:
            ...
            print(f"Updated column names: {self.top_listbox_edits}")
            self.top_listbox_edits.clear()  # Clear the tracked changes after applying

        for col_name, changes in self.bottom_listbox_edits.items():
            # Avoid inplace=True, instead assign the result back to the DataFrame column
            self.df[col_name] = self.df[col_name].replace(changes)
            print(f"Updated unique values in column '{col_name}': {changes}")

        self.bottom_listbox_edits.clear()  # Clear the tracked changes after applying
